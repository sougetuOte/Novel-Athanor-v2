...
- 検証結果は `docs/memos/walkthrough-<feature>.md` にまとめ、スクリーンショットやログと共に報告することを推奨する。

### Automated TDD Introspection (v4.0.0)

PostToolUse hook がテスト実行結果を自動監視し、以下を記録する:
- テスト失敗/成功パターンを `.claude/tdd-patterns.log` に蓄積
- `src/` 配下のファイル変更を `.claude/doc-sync-flag` に記録（`/ship` の Doc Sync チェックで使用）
