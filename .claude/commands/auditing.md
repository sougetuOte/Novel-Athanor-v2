...
### Python 安全性（`.claude/rules/phase-auditing.md` / R-7〜R-9 参照）
...
### 既存ルール適用漏れ点検（A-6）
- [ ] `.claude/rules/building-checklist.md` の R-1〜R-12 について grep/検索ベースで違反が残っていないか確認（目視のみの確認は禁止）
...
**A-6**: `building-checklist.md` の全ルール（R-1〜R-12）について grep/検索ベースで違反が残っていないか確認する。
...
- .claude/rules/audit-fix-policy.md (A-1〜A-6)
- .claude/rules/phase-auditing.md
