...
対応仕様: docs/internal/07_SECURITY_AND_AUTOMATION.md Section 5（PostToolUse による自動記録）
"""
...
def _handle_test_result(
    ...
    if last_result_file.exists():
        try:
            ...
            # OSError: ファイル読取失敗、IndexError: 空ファイル。prev_result="" のまま続行。
            pass

    last_result_file.parent.mkdir(parents=True, exist_ok=True)

    # exit_code は文字列で渡される（"0", "1" 等）。
    # 空文字 = exit code 未取得（Bash 以外のツール等）→ スキップ
...
def _handle_loop_log(
    ...
    try:
        ...
        return  # パース失敗時は追記をスキップ（破損ファイルの上書き防止）
