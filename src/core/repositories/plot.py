...
Plot エンティティの CRUD 操作を行うリポジトリ。
3階層のプロット（L1/L2/L3）を統一的に扱う。
"""
...
class PlotRepository(BaseRepository[Plot]):
    ...
    Note:
        L3 plots の場合、chapter_name が PlotL3 モデルに含まれていないため、
    ...
    def _get_path(self, identifier: str) -> Path:
        ...
        Returns:
            ファイルパス
        """
        parts = identifier.split("-", maxsplit=3)  # "L3-1-Chapter One-1" -> ["L3", "1", "Chapter One", "1"]
        level = parts[0]

        if level == "L1":
            return self._path_resolver.resolve_plot("L1")
        elif level == "L2":
    ...
    def _get_identifier(self, entity: Plot) -> str:
        """プロットから識別子を生成.

        Note:
            L3 の場合、chapter_name が必要だが PlotL3 モデルには含まれていない。
    ...
    def get_by_level(self, level: Literal["L1", "L2", "L3"]) -> list[Plot]:
        ...
        """
        plots = self.list_all()
        return [p for p in plots if p.level == level]

    def read(self, identifier: str) -> Plot:
        """エンティティを読み込み.
    ...
    def _get_chapter_name_for_l3(self, chapter_number: int) -> str:
        ...
        """
        # まず L2 から探す
        l2_dir = self.vault_root / "_plot" / "L2_chapters"
        if l2_dir.exists():
        ...
        l3_dir = self.vault_root / "_plot" / "L3_sequences"
        if l3_dir.exists():
    ...
    def list_all(self) -> list[Plot]:
        ...
        # L1: _plot/L1_overall.md
        l1_path = plot_dir / "L1_overall.md"
        if l1_path.exists():
            plots.append(self._read(l1_path))

        # L2: _plot/L2_chapters/*.md
        l2_dir = plot_dir / "L2_chapters"
        if l2_dir.exists():
            for path in l2_dir.glob("*.md"):
                plots.append(self._read(path))

        # L3: _plot/L3_sequences/*/*.md
        l3_dir = plot_dir / "L3_sequences"
        if l3_dir.exists():
            for chapter_dir in l3_dir.iterdir():
